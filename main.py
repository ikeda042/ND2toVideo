...
def add_scale_bar(image_path, scale_length_um=10) -> Image.Image:
    ...
    draw.rectangle([bar_x, bar_y, bar_x + bar_width_pixels, bar_y + bar_height], fill="white")
...
def extract_nd2(file_name: str, target_view: int):
    ...
    with nd2reader.ND2Reader(file_name) as images:
        ...
        if 'v' not in images.axes:
            raise ValueError("The specified ND2 file does not contain 'v' axis for multiple views.")
    ...
    for i in range(len( [os.path.join("nd2totiff/", f) for f in os.listdir("nd2totiff/") if f.endswith('.tif')])):
...
def convert_to_video() -> None:
    ...
    out = cv2.VideoWriter(output_video_path, cv2.VideoWriter_fourcc('M','J','P','G'), 5, (frame_width, frame_height))
